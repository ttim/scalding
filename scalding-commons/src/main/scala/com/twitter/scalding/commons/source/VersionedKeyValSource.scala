...
import com.twitter.scalding.typed.{KeyedListLike, TypedSink}
...
object RichPipeEx extends java.io.Serializable {
  ...
  implicit def typedPipeToRichPipeEx[K: Grouping, V: Monoid](pipe: TypedPipe[(K, V)]): TypedRichPipeEx[K, V] =
    new TypedRichPipeEx(pipe)
  implicit def keyedListLikeToRichPipeEx[K: Grouping, V: Monoid, T[K, +V] <: KeyedListLike[K, V, T]](
    kll: KeyedListLike[K, V, T]): TypedRichPipeEx[K, V] = typedPipeToRichPipeEx(kll.toTypedPipe)
}

class TypedRichPipeEx[K: Grouping, V: Monoid](pipe: TypedPipe[(K, V)]) extends java.io.Serializable {
  import Dsl._
  import TDsl._
