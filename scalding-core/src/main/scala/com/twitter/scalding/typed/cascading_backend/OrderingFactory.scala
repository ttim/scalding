...
object OrderingFactory {
  ...
  def toOrderedSerialization[T](
    ...
    (grouping match {
      ...
      //      case ForTuple2(g1, g2) =>
      //        for (
      //          ordSer1 <- toOrderedSerialization(g1);
  ...
  //
  //  private def tuple3OrdSer[T1: OrderedSerialization, T2: OrderedSerialization, T3: OrderedSerialization]: OrderedSerialization[(T1, T2, T3)] =
